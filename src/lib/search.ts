// Country / US-state name search with diacritic folding and prefix ranking.
...
export function searchEntities(query: string, options: SearchOptions = {}): SearchResult[] {
  ...
  for (const c of countries) {
    ...
    if (score > 0) results.push({ entity: countryEntity(c), score, sub: c.region })
  }
  for (const s of states) {
    const score = scoreCandidate(q, s.name, [], s.code)
    if (score > 0) results.push({ entity: stateEntity(s), score, sub: 'US state' })
  }
